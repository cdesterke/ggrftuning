#' Plot Variable Importance with Custom Viridis Palette (Robust Version)
#'
#' This function extracts variable importance measures from a fitted
#' randomForest model and generates ggplot2 barplots using a chosen
#' viridis color palette. It automatically adapts to classification or
#' regression models and never fails if some importance metrics are missing.
#'
#' @param model_rf A fitted randomForest model.
#' @param base_size Numeric. Base font size for ggplot themes. Default = 16.
#' @param palette Character. Viridis palette name:
#'   "magma", "viridis", "inferno", "plasma", "cividis",
#'   "rocket", "mako", "turbo", or "volcano".
#'   Default = "magma".
#'
#' @return A list with:
#' \describe{
...
#'   )
#'
#'   # Generate importance plots with a chosen palette
...
#'
#'   # Access a specific plot (e.g., MeanDecreaseGini)
#'   out_imp$plots$MeanDecreaseGini
#'
#'   # Access MeanDecreaseAccuracy if available
#'   out_imp$plots$MeanDecreaseAccuracy
#'
#'   # Display all available plots
#'   for (p in names(out_imp$plots)) {
#'     print(out_imp$plots[[p]])
#'   }
#' }
#'
#' @export
rf_importance_plots <- function(model_rf, base_size = 16,
  ...
  # Supported viridis palettes
  viridis_palettes <- c("magma", "viridis", "inferno", "plasma",
                        "cividis", "rocket", "mako", "turbo")

  # Handle "volcano" gracefully
  if (palette == "volcano") {
    message("Palette 'volcano' is not a viridis option. Using 'inferno' as a close alternative.")
    palette <- "inferno"
  }

  # Validate palette
  if (!palette %in% viridis_palettes) {
    stop(paste0("Palette '", palette, "' is not supported. Choose among: ",
                paste(viridis_palettes, collapse = ", "), ", volcano"))
  }
  ...
  # Function to generate a barplot for one metric
  make_plot <- function(df, metric) {
    ...
    ggplot2::ggplot(
      df,
      ggplot2::aes(
        x = reorder(.data$Variable, .data[[metric]]),
        y = .data[[metric]],
        fill = .data[[metric]]
      )
    ) +
