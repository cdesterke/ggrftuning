...
#' model performance and a ggplot2 visualization showing the evolution of
#' out-of-bag (OOB) error across trees, both globally and for each class.
#'
...
#'   X <- iris[, 1:4]
#'   Y <- iris$Species
#'   out <- rf_model_with_oob(X, Y, mtry = 2, ntree = 50, split = 0.65)
#'   out$results
#'   out$plot
#'   out$model
#' }
